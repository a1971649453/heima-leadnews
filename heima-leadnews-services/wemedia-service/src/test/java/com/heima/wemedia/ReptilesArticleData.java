...
//                .get();
        // 找到指定class的所有元素
        Elements flowList = document.getElementsByClass("widget-flowList");
        Element element = flowList.get(0);
        Elements articleList = element.getElementsByTag("li");
        ...
        // 找到所有article标签页面元素
            for (Element article : articleList) {
                ...
                try {
                    ...
                    Element titleEle = aElement.getElementsByClass("s-left").get(0);
                    Elements tiltes = titleEle.getElementsByTag("h4");
                    for (Element tilte1 : tiltes) {
                        title = tilte1.text();
                    }
                    System.out.println("新闻的文章标题:"+title);
                    // 获取封面图片元素集合

                    Element newsPic = aElement.getElementsByClass("s-right").get(0);
                    imgList = newsPic.getElementsByTag("img");
                    //如果有图片集合的情况下
                    Elements sImgs = aElement.getElementsByClass("s-imgs");
                    if (sImgs.size() != 0) {
                        imgList = sImgs.get(0).getElementsByTag("img");
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                    continue;
